package frc.robot.pose;

import edu.wpi.first.math.geometry.Translation2d;
import java.util.Objects;

/**
 *
 * @author aheitkamp
 */
public class TargetNode {
    
    public static TargetNode A1 = new TargetNode(0,0,NodeID.A1);
    public static TargetNode A2 = new TargetNode(0,0,NodeID.A2);
    public static TargetNode A3 = new TargetNode(0,0,NodeID.A3);
    public static TargetNode B1 = new TargetNode(0,0,NodeID.B1);
    public static TargetNode B2 = new TargetNode(0,0,NodeID.B2);
    public static TargetNode B3 = new TargetNode(0,0,NodeID.B3);
    public static TargetNode NONE = new TargetNode(0,0,NodeID.NONE);
    
    public static enum NodeID {
        A1,
        A2,
        A3,
        B1,
        B2,
        B3,
        NONE
    }    
    
    private final Translation2d offsetToTarget;
    private final NodeID id;

    protected TargetNode(double horizontalOffsetInches, double verticalOffsetInches ,NodeID ID) {
        offsetToTarget = new Translation2d(horizontalOffsetInches,verticalOffsetInches);
        id = ID;
    }

    public NodeID getNodeID() { 
        return id; 
    }    
    
    public Translation2d getOffsetToTarget() {
        return offsetToTarget; 
    }


    public boolean isThirdRow(){
        return (id == NodeID.A1 || id == NodeID.A2 || id == NodeID.A3);
    }
    public boolean is2ndRow(){
        return (id == NodeID.B1 || id == NodeID.B2 || id == NodeID.B3);
    }    
    public boolean isTargetSelected(){
        return id != NodeID.NONE;
    }
    public boolean noTargetSelected(){
        return id == NodeID.NONE;
    }
    @Override
    public boolean equals(Object o) {
        if (o == this)
            return true;
        if (!(o instanceof TargetNode)) {
            return false;
        }
        TargetNode nodePose = (TargetNode) o;
        return Objects.equals(offsetToTarget, nodePose.offsetToTarget) && Objects.equals(id, nodePose.id);
    }

    @Override
    public int hashCode() {
        return Objects.hash(offsetToTarget, id);
    }    
    
}
