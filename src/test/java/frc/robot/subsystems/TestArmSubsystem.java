...
import frc.robot.controllers.MockPositionController;
...
//		PositionControllerConfig config = new PositionControllerConfig.Builder("test").withHomingOptions(10, 20, 20)
//				.withPositionTolerance(2).withReversed(false).withSoftLimits(25, 200).build();
//		MockPositionController tp = new MockPositionController(config);
//		
...
//		periodicSteps(as,2);
//		
//		assertEquals(MockPositionController.POSITION_UNKNOWN, tp.getActualPosition());
...
//		MockPositionController tp = new MockPositionController();
//		tp.setLowerLimitTripped(true);
//		CANSparkMax fakeArm = mock(CANSparkMax.class, Mockito.RETURNS_DEEP_STUBS);
//		ArmSubsystem as = new ArmSubsystem(tp,fakeArm);		
